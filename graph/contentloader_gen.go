...
type ContentLoaderConfig struct {
	...
	Fetch func(keys []string) ([]*model.Content, []error)
...
type ContentLoader struct {
	...
	cache map[string]*model.Content
...
func (l *ContentLoader) Load(key string) (*model.Content, error) {
...
func (l *ContentLoader) LoadThunk(key string) func() (*model.Content, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*model.Content, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *ContentLoader) LoadAll(keys []string) ([]*model.Content, []error) {
	results := make([]func() (*model.Content, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	contents := make([]*model.Content, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *ContentLoader) LoadAllThunk(keys []string) func() ([]*model.Content, []error) {
	results := make([]func() (*model.Content, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *ContentLoader) Prime(key string, value *model.Content) bool {
	l.mu.Lock()
	var found bool
...
func (l *ContentLoader) unsafeSet(key string, value *model.Content) {
	if l.cache == nil {
		l.cache = map[string]*model.Content{}
	}
	l.cache[key] = value
}
